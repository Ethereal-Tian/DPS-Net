...
def read_gen(file_name, pil=False, channels=3):
    ...
    # elif ext == '.exr':
    #     return readExr(file_name).astype(np.float32)
    elif ext == '.pfm':
        flow = readPFM(file_name, channels=channels).astype(np.float32)        
        return flow
        # if len(flow.shape) == 2:
        #     return flow
        # else:
        #     return flow[:, :, :-1]
        # normal = readPFM(file_name).astype(np.float32)
        # if len(normal.shape) == 3:
        #     return normal 
        # else:
        #     print(f"normal image channel should be 3, not {normal.shape}")
        #     os.system("pause")
    return []
