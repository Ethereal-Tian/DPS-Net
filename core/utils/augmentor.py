...
class SimpleAugmentor:
    ...
    def spatial_transform(self, img1, img2, pol1, pol2, flow):
        # randomly sample scale
        ...
        img1 = img1[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        img2 = img2[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        pol1 = pol1[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        pol2 = pol2[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        flow = flow[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        return img1, img2, pol1, pol2, flow ,[x0,y0]

    def __call__(self, img1, img2, pol1, pol2, flow):
        img1, img2, pol1, pol2, flow ,[x0,y0] = self.spatial_transform(img1, img2, pol1, pol2, flow)

        img1 = np.ascontiguousarray(img1)
        img2 = np.ascontiguousarray(img2)
        pol1 = np.ascontiguousarray(pol1)
        pol2 = np.ascontiguousarray(pol2)
        flow = np.ascontiguousarray(flow)

        return img1, img2, pol1, pol2, flow ,[x0,y0]

class SparseSimpleAugmentor:
    def __init__(self, crop_size):
        # spatial augmentation params
        self.crop_size = crop_size

    def spatial_transform(self, img1, img2, pol1, pol2, flow, valid):
        # randomly sample scale
        margin_y = 20
        margin_x = 50
        ...
        # print("rand offset",x0,y0)

        y0 = np.clip(y0, 0, img1.shape[0] - self.crop_size[0])
        x0 = np.clip(x0, 0, img1.shape[1] - self.crop_size[1])

        # print("dolp1[480][320]",pol1[480][320][0])
        # print("aolp1[480][320]",pol1[480][320][1])
        # print("dolp1[320][480]",pol2[320][480][0])
        # print("aolp1[320][480]",pol2[320][480][1])
        ...
        return img1, img2, pol1, pol2, flow, valid,[x0,y0]

    def __call__(self, img1, img2, pol1, pol2, flow, valid):
        img1, img2, pol1, pol2, flow, valid, [x0,y0] = self.spatial_transform(img1, img2, pol1, pol2, flow, valid)
...
class SimpleCPSAugmentor:
    def __init__(self, crop_size):
        # spatial augmentation params
        self.crop_size = crop_size

    def spatial_transform(self, img1, img2, stoke1, stoke2, pol1, pol2, flow):
        # randomly sample scale
        ...
        pol1 = pol1[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        pol2 = pol2[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        stoke1 = stoke1[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        stoke2 = stoke2[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        flow = flow[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        return img1, img2, stoke1, stoke2, pol1, pol2, flow ,[x0,y0]

    def __call__(self, img1, img2, stoke1, stoke2, pol1, pol2, flow):
        img1, img2, stoke1, stoke2, pol1, pol2, flow ,[x0,y0] = self.spatial_transform(img1, img2, stoke1, stoke2, pol1, pol2, flow)

        img1 = np.ascontiguousarray(img1)
        img2 = np.ascontiguousarray(img2)
        pol1 = np.ascontiguousarray(pol1)
        pol2 = np.ascontiguousarray(pol2)
        stoke1 = np.ascontiguousarray(stoke1)
        stoke2 = np.ascontiguousarray(stoke2)
        flow = np.ascontiguousarray(flow)

        return img1, img2, stoke1, stoke2, pol1, pol2, flow ,[x0,y0]

class SparseSimpleCPSAugmentor:
    def __init__(self, crop_size):
        # spatial augmentation params
        self.crop_size = crop_size

    def spatial_transform(self, img1, img2, stoke1, stoke2, pol1, pol2, flow, valid):
        # randomly sample scale
        margin_y = 20
        margin_x = 50
        ...
        return img1, img2, stoke1, stoke2, pol1, pol2, flow, valid,[x0,y0]

    def __call__(self, img1, img2, stoke1, stoke2, pol1, pol2, flow, valid):
        img1, img2, stoke1, stoke2, pol1, pol2, flow, valid, [x0,y0] = self.spatial_transform(img1, img2, stoke1, stoke2, pol1, pol2, flow, valid)
